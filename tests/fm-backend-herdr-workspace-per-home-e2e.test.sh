...
FM_SPAWN_NO_GUARD=1 FM_HOME="$PRIMARY_HOME" FM_ROOT_OVERRIDE="$ROOT" \
  "$ROOT/bin/fm-spawn.sh" cm1 "$PROJ1" "bash -c 'echo primary-crew-ok; read -r _'" --backend herdr \
  >"$CM1_OUT" 2>"$CM1_ERR"
rc=$?
...
FM_SPAWN_NO_GUARD=1 FM_HOME="$PRIMARY_HOME" FM_ROOT_OVERRIDE="$ROOT" \
  "$ROOT/bin/fm-spawn.sh" e2esm1 "$SM_HOME" "bash -c 'echo secondmate-launch-ok; read -r _'" --secondmate --backend herdr \
  >"$SM_OUT" 2>"$SM_ERR"
rc=$?
...
FM_SPAWN_NO_GUARD=1 FM_HOME="$SM_HOME" FM_ROOT_OVERRIDE="$ROOT" \
  "$ROOT/bin/fm-spawn.sh" cm2 "$PROJ2" "bash -c 'echo sm-crew-ok; read -r _'" --backend herdr \
  >"$CM2_OUT" 2>"$CM2_ERR"
rc=$?
