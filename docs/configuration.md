...
FM_BACKEND_HERDR_READY_POLLS=100  # herdr-only: maximum process/cwd and exact execution-ack samples at each interactive-shell readiness boundary
FM_BACKEND_HERDR_READY_STABLE_POLLS=20  # herdr-only: consecutive shell-only process/cwd samples required before the execution canary is sent
FM_BACKEND_HERDR_READY_POLL_SLEEP=0.1  # herdr-only: seconds between shell-readiness and launch-handoff samples
FM_BACKEND_HERDR_HANDOFF_POLLS=200  # herdr-only: maximum samples for exact launch witness plus verified worker process/agent evidence
FM_HERDR_PRESENTATION_LOCK_POLLS=1500  # herdr projected-spawn-only: maximum named-session lock attempts spanning creation through verified handoff
FM_HERDR_PRESENTATION_LOCK_SLEEP=0.1  # herdr projected-spawn-only: seconds between named-session presentation-lock attempts
